PRO time_series::select, years=years,months=months,days=days, $
                         hours=hours,minutes=minutes,seconds=seconds, $
                         all=all,select=select,none=none, $
                         tags=tags,count=count,ind_selected=ind_selected,period=period
;
; this allows to select fields within the object
;
...
;
    ...
    IF KEYWORD_SET(all) THEN BEGIN
	  (*self.selected)=(*self.selected)*0.0 + 1
	  RETURN
	ENDIF
    ...
    IF KEYWORD_SET(select) THEN BEGIN
	  IF N_ELEMENTS(select) NE self.nfields THEN BEGIN  
		...
		RETURN
	  ENDIF
	  (*self.selected)[0:N_ELEMENTS(select)-1]=select
	  RETURN
	ENDIF
    ...
    FOR i=0L,self->get_count()-1 DO BEGIN
	  ...
	  dum = WHERE(STRUPCASE((*self.tag)[i]) EQ STRUPCASE(tags)  ,c6)
  	  IF c0+c1+c2+c3+c4+c5+c6 EQ 7 THEN select[i] = 1 ELSE select[i] = 0	 
    ...
    IF KEYWORD_SET(period) THEN BEGIN
      ...
      FOR i=0L,self->get_count()-1 DO BEGIN
        ...
        IF c1 GT 0 AND jul[i] GE period[0] AND jul[i] LE period[1] THEN select[i]=1
      ENDFOR
    ENDIF
