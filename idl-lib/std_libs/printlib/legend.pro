...
;               defined user symbol.  If the keyword SYM is set, use the symbols in CSYM.
...
;-
pro legend,help=help,items,linestyle_=linestyle_,psym_=psym_,vectorfont=vectorfont $
  ...
  ,position=position,number=number,colors_=colors_,textcolors_=textcolors_ $
  ...
  ,data=data,normal=normal,device=device $
   ,symsize_=symsize_,thick_=thick_,clear=clear, csym_ = csym_, $
   symthick_ = symthick_
;
;       =====>> HELP
;
...
;********************************
;PART ADDED BY CHRIS O'DELL SO PRO DOESN'T CHANGE KEYWORD VALUES
if keyword_set(linestyle_) then linestyle = linestyle_
if keyword_set(colors_) then colors = colors_
if keyword_set(thick_) then thick = thick_
if keyword_set(psym_) then psym = psym_
if keyword_set(textcolors_) then textcolors = textcolors_
if keyword_set(symthick_) then symthick = symthick_
if keyword_set(symsize_) then symsize = symsize_
;********************************
...
nst = n_elements(symthick)
nss = n_elements(symsize)
nlpv = max([np,nl,nv])
...
symline = (np ne 0) or (nl ne 0)                        ; FLAG TO PLOT SYM/LINE
 ...
 if nst EQ 0 then symthick = intarr(n)+1 else $
         if nst EQ 1 then symthick = intarr(n) + symthick
 if nss EQ 0 then symsize = intarr(n) + 1 else $
         if nss EQ 1 then symsize = intarr(n) + symsize
...
;; extra by djseed .. psym=88 means use the already defined usersymbol
 ...
 if keyword_set(csym_) then psym[i] = csym(psym[i], thick = symthick[i])
...
;    if (num eq 1) and vertical then xp = x + xt/2      ; IF 1, CENTERED.
  ...
  TEXT_ONLY:
  xyouts,x,y,delimiter,width=width,/norm,color=textcolors[i],charsize=charsize,align=xalign
  x = x + width*xsign
  if width ne 0 then x = x + 0.5*xspacing
  xyouts,x,y,items[i],width=width,/norm,color=textcolors[i],charsize=charsize,align=xalign
  x = x + width*xsign
