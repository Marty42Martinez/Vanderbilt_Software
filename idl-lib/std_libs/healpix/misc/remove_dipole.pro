...
if undefined(bad_data) then bad_data = -1.63750000e+30


dow8 = 0
if (obs_npix2 gt 1) then begin
    if (obs_npix2 ne obs_npix) then begin
        print,code+': inconsistent map and weight'
        stop
    endif else begin
        dow8 = 1
    endelse
...
if (obs_npix3 gt 1) then begin
    if (obs_npix3 ne obs_npix) then begin
        print,code+': inconsistent map and pixel'
        stop
    endif else begin
        partial = 1
    endelse
...
if (error_coord1+error_coord2) ne 0 then begin
    print,code+': invalid choice of coordinate'
    stop
endif
...
npiece = 24
if (not dodipole and not galcut) then npiece = 4
...
for is = 0,npiece do begin
    ...
    ; pixel weight
    flag = abs(map[imin:imax]/bad_data-1.) gt .01   ; byte array, either 1 for good pixels or 0 for bad ones
...
; do a loop to minimize arrays size
    for i=0,npiece do begin
        ...
        good = imin + where( abs(map[imin:imax]/bad_data-1.) gt .01, ngood )
