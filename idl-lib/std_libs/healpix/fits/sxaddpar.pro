...
;		record is added to the header.  If name = 'HISTORY' or
;		'COMMENT' then the 
...
;	
;       Accept multiple COMMENT's field EH, Caltech, March 1999
;-
...
;;;;;; if nn eq 'HISTORY ' then begin             ;add history record?
 if nn eq 'HISTORY ' or nn eq 'COMMENT ' then begin ;add history or comment record? modified by EH 03/99
      if iend lt (n-1) then $
...
;;;;;             if keyword_set(after) and loc ne 'HISTORY ' then i = i+1 < iend 
             if keyword_set(after) and loc ne 'HISTORY ' and loc ne 'COMMENT ' then i = i+1 < iend ; modified by EH
