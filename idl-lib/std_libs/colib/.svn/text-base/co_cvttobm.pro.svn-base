...
function co_cvttobm, mask, new=new

	; input # of elements along first dimension must be evenly divisible by 8

	; the chris way
	sm = size(mask)
	dims = sm[1:sm[0]]
	dims[0] = dims[0] / 8
	bmp = make_array(dims, /byte)

	two_n = [1b,2b,4b,8b,16b,32b,64b,128b]
	for n=0,7 do bmp = bmp + mask[n:*:8,*] * two_n[n]
