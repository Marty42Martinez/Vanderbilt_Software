...
;     burton s. garbow, kenneth e. hillstrom, jorge j. more
;
pro mpfit_qrsolv, r, ipvt, diag_, qtb, x, sdiag
...
;  prof_start = systime(1)
  ...
  for j = 0L, n-1 do begin
      l = ipvt(j)
      if diag_(l) EQ 0 then goto, STORE_RESTORE
      sdiag(j:*) = 0
      sdiag(j) = diag_(l)
...
;     the subroutine statement is
;
;	subroutine lmpar(n,r,ldr,ipvt,diag_,qtb,delta,par,x,sdiag,
;			 wa1,wa2)
;
;     where
;
...
;	  is column ipvt(j) of the identity matrix.
;
;	diag_ is an input array of length n which must contain the
;	  diagonal elements of the matrix d.
;
;	qtb is an input array of length n which must contain the first
;	  n elements of the vector (q transpose)*b.
...
;     burton s. garbow, kenneth e. hillstrom, jorge j. more
;
function mpfit_lmpar, r, ipvt, diag_, qtb, delta, x, sdiag, par=par
...
;  prof_start = systime(1)
  ...
  if nsing GE n then begin
      wa1 = diag_(ipvt)*wa2(ipvt)/dxnorm
...
;			 diag_,mode,factor,nprint,info,nfev,fjac,
...
;	  precision.
;
;	diag_ is an array of length n. if mode = 1 (see
;	  below), diag_ is internally set. if mode = 2, diag_
;	  must contain positive entries that serve as
;	  multiplicative scale factors for the variables.
;
;	mode is an integer input variable. if mode = 1, the
;	  variables will be scaled internally. if mode = 2,
;	  the scaling is specified by the input diag_. other
;	  values of mode are equivalent to mode = 1.
;
;	factor is a positive input variable used in determining the
;	  initial step bound. this bound is set to the product of
;	  factor and the euclidean norm of diag_*x if nonzero, or else
...
function mpfit, fcn, xall, FUNCTARGS=fcnargs, SCALE_FCN=scalfcn, $
                ...
                diag_=diag_, rescale=rescale, autoderivative=autoderiv, $
  ...
  if keyword_set(rescale) then begin
      ...
      if n_elements(diag_) LT n then goto, TERMINATE
      wh = where(diag_ LE 0, ct)
...
;  prof_start = systime(1)
  ...
  ;; On the first iteration if "diag_" is unspecified, scale
  ;; according to the norms of the columns of the initial jacobian
  catch_msg = 'rescaling diagonal elements'
  if (iter EQ 1) then begin

      if NOT keyword_set(rescale) OR (n_elements(diag_) LT n) then begin
          diag_ = wa2
          wh = where (diag_ EQ 0, ct)
          if ct GT 0 then diag_(wh) = one
      endif
  ...
  par = mpfit_lmpar(fjac, ipvt, diag_, qtf, delta, wa1, wa2, par=par)
...
;  profvals.mpfit = profvals.mpfit + (systime(1) - prof_start)
  ...
  varlist = ['alpha', 'delta', 'diag_', 'dwarf', 'factor', 'fnorm', $
