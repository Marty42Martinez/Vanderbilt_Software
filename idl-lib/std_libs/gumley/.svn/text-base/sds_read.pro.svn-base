...
if nsds ge 0 then begin
  ; the if statement below has been added b/c the program crashes,
  ; if nsds EQ 0 b/c then sds_id is undefined. RB 4/21/2010
  IF nsds GT 0 THEN $ 
...
if n_elements( att_name ) gt 0 then $
  loc = where( att_name eq 'scale_factor' or att_name eq 'slope', scale_count )
if scale_count eq 1 then begin
  att_id = hdf_sd_attrfind( sds_id, 'scale_factor' )
;   att_id = hdf_sd_attrfind( sds_id, 'slope' )
...
;  loc = where( att_name eq 'add_offset', offset_count )
  loc = where( att_name eq 'intercept', offset_count )
if offset_count eq 1 then begin
;  att_id = hdf_sd_attrfind( sds_id, 'add_offset' )
   att_id = hdf_sd_attrfind( sds_id, 'intercept' )
