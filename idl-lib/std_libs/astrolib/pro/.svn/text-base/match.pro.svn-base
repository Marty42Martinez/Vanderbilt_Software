...
;
; INPUTS:
;       a,b - two vectors to match elements, can be any numeric datatype
;
; OUTPUTS:
...
;-------------------------------------------------------------------------
 ...
 if keyword_set(sort) then fpoint = 1b else $
 fpoint = ( da EQ 4 ) or (da EQ 5) or (db EQ 4) or (db EQ 5 )

 if fpoint then begin           ;Non-integer calculation
