...
;
; CALLING SEQUENCE:
;     CO_NUTATE, jd, ra, dec, d_ra, d_dec, [EPS=, D_PSI =, D_EPS = , /FAST]
;
; INPUTS
...
;	 FAST : assume all jd's are the same.  this is fine for observations
;			taken within a few days of each other, shouldn't make much of a difference.
...
PRO co_nutate, jd, ra, dec, d_ra, d_dec, eps=eps, d_psi=d_psi, d_eps=d_eps, fast=fast
...
if keyword_set(fast) then nutate, jd[0], d_psi, d_eps else nutate,jd,d_psi, d_eps
