...
;                       ABERRATION_= 0, ALTITUDE= , /VERBOSE,
;						P_ANGLE = P_ANGLE, /FAST, _EXTRA= ]
;
;
; INPUT VARIABLES
...
;		p_angle : Set this to a named variable to contain the parallactic angle (degrees).
;		fast 	: Set this to do a "fast" nutation and precession calculation.
;				  Doing this assumes that all observations occurred roughly
;				  at the same time (to within a few days for subarcsecond precision).
...
;
; UPDATES:
;	- 12/02/2002 : Corrected slight bugs with vector inputs.
; 	- 12/02/2002 : Addition of FAST keyword for faster vector calculations.
;
; AUTHOR:
;   Chris O'Dell
...
if v then print, 'Latitude = ', adstring(lat[0]), '   Longitude = ', adstring(lon[0])
if v then print, 'Julian Date = ', jd[0], format='(A,f15.6)'
if v then print,'Az, El = ', adstring(az_[0], alt_[0]), '   (Observer Coords)'
...
if v then print,'Az, El = ', adstring(az_[0], alt_[0]), '   (Apparent Coords)'
...
co_nutate, jd, 45.,45., dra1, ddec1, eps=eps, d_psi=d_psi, fast=fast ; get rough nutation correction factors
last = lmst + d_psi *cos(eps)/3600. ; add correction in degrees
if v then print, 'LMST = ', adstring(lmst[0]/15.)
if v then print, 'LAST = ', adstring(last[0]/15.)

;****************************************************************************
; Now do the spheical trig to get APPARENT Hour Angle [degrees], and
; declination [degrees], and the optional parallactic angle [degrees].
altaz2hadec, alt_, az_, lat, ha, dec, p_angle

; Find Right Ascension (in degrees, from 0 to 360.)
 ra = (last - ha + 360.) mod 360.

if v then print, 'Hour Angle = ', adstring(ha[0]/15.), '  (hh:mm:ss)'
if v then print, 'Ra, Dec: ', adstring(ra[0],dec[0]), '   (Apparent Coords)'


;*****************************************************************************
; calculate NUTATION and ABERRATION Corrections to Ra-Dec
co_nutate, jd, ra, dec, dra1, ddec1, eps=eps, d_psi=d_psi, fast=fast
co_aberration, jd, ra, dec, dra2, ddec2, eps=eps, fast=fast
...
if v then print, 'Ra, Dec: ', adstring(ra[0],dec[0]), '   (J'+ $
           strcompress(string(J_now[0]),/rem)+')'
...
if precess_ then begin
	if not(keyword_set(fast)) then begin
	    ...
	    endelse
	endif else begin ; do "fast" precession calculation
		if keyword_set(B1950) then precess, ra,dec,J_now[0], 1950.0, /FK4 $
			else precess, ra, dec, J_now[0], 2000.0
	endelse
endif

if keyword_set(B1950) then s_now='   (J1950)' else s_now='   (J2000)'
if v then print, 'Ra, Dec: ', adstring(ra[0],dec[0]), s_now[0]
