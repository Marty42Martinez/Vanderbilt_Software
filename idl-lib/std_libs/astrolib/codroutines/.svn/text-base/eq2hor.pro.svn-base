...
;                       ABERRATION_= 0, ALTITUDE= , /FAST, /VERBOSE, _EXTRA= ]
;
; DESCRIPTION:
...
pro eq2hor, ra, dec, jd, alt, az, ha, lat=lat, lon=lon, WS=WS, obsname=obsname,$
     B1950 = B1950, verbose=verbose, precess_=precess_, nutate_=nutate_, $
                ...
                altitude = altitude, fast=fast, _extra= _extra
...
if (n_elements(ra) LT n_elements(jd)) then begin
; this should be the case for a single ra, dec, but multiple julian dates.
	ra_ = ra_[0] + jd*0.
	dec_ = dec_[0] + jd*0.
endif
...
if v then print, 'Ra, Dec: ', adstring(ra_[0],dec_[0]), s_now[0]
...
if precess_ then begin
	if not(keyword_set(fast)) then begin   ; do "slow" precession calculation
			...
			if keyword_set(B1950) then precess, ra,dec,1950.0,mean(J_now), /FK4 $
			else precess, ra_, dec_, 2000.0, mean(J_now)
	endelse
endif

if v then print, 'Ra, Dec: ', adstring(ra_[0],dec_[0]), '   (J' + $
          strcompress(string(J_now[0]),/rem)+')'


;******************************************************************************
; calculate NUTATION and ABERRATION Corrections to Ra-Dec
co_nutate, jd, ra_, dec_, dra1, ddec1, eps=eps, d_psi=d_psi, fast=fast
co_aberration, jd, ra_, dec_, dra2, ddec2, eps=eps, fast=fast
...
if v then print, 'Ra, Dec: ', adstring(ra_[0],dec_[0]), '   (fully corrected)'
...
if v then print,'Az, El = ', adstring(az[0],alt[0]), '   (Apparent Coords)'
...
if v then print,'Az, El = ', adstring(az[0],alt[0]), '   (Observer Coords)'
