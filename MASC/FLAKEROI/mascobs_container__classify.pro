...
PRO mascobs_container::wdisplay,i,w,xgam,init=init,done=done,pretty=pretty
     ...
     ENDIF ELSE BEGIN
	    if keyword_set(pretty) then begin
		  w.erase
		  rgb_table=COLORTABLE(49,/REVERSE)
          g = image(c, current=w,name='canvas',rgb_table=rgb_table)
          g.scale, 2.0, 2.0, 1.0
          PRINT,'Flake No : ',i
		endif else begin
          w.erase
...
PRO mascobs_container__classify_event,event
   ...
   CASE res OF
	  ...
	  'Pretty'   : BEGIN
                     ;xgam=xgam/0.9
                     calling_object->wdisplay,icurrentflake,w,xgam,init=1,/pretty
                   END
      'Classify' : BEGIN
...
FUNCTION mascobs_container::classify,categories=categories,outfile=outfile,redo=redo
   ...
   ; crate top widget
   wtop  = widget_base( column=3,  title='Classify Test')
   ...
   bpretty = WIDGET_BUTTON( wtop, VALUE = "Pretty", XSIZE=50,YSIZE=50 , uname='Pretty')
